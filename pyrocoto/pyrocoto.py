...
class CycleDefinition():
    ...
    def __eq__(self,other):
        if isinstance(other , CycleDefinition):
            return (self.group == other.group and self.definition == other.definition and self.activation_offset == other.activation_offset)
        else:
            return False
...
class Workflow(object):
    ...
    ''' Implement an abstarction layer on top of rocoto workflow management engine
        The WorkFlow class will serve as a central object that registers all units of work (tasks)
        for any number of desired cycle definitions.
    '''
    ...
    def __init__(self, **kwargs):
        ...
        self.workflow_element = Element('workflow', realtime=realtime, scheduler=scheduler, **kwargs)
    ...
    def register_task(self, groups=None, task_name=None, task=None, meta=None, metaname=None):
    ...
    def write_xml(self, xmlfile, groups=None, cycledefs=None):
        ''' validate that registered workflow tasks and cycles are valid and write xml workflow
            groups of tasks or tasks based on specific cycle definitions can be spacifically written
        ...
        for ix, task in enumerate(self.task_data):
            ...
            for key in task.keys():
                ...
                if key not in ['command', 'account', 'queue', 'cores', 'nodes', 'walltime', 'envar', 'native',
                               'memory', 'jobname', 'join', 'stdout', 'stderr', 'deadline', 'dependency']:
                    continue
...
class Task():
    ...
    def verify(self):
        required_keys = [['command'],['join','stderr',],['cores','nodes'],['cycledefs']]
